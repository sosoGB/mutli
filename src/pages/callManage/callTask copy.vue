  ...
  <div class="callList page-list">
    <div class="toolbar">
      <div class="tool-search">
        ...
        <el-date-picker v-model="search.lastUpdateBeginTime" class="search-component search-input" type="date" placeholder="更新开始时间" value-format="yyyy-MM-dd" :picker-options="beginUpdateValidator" :clearable="false" @change="checkEndDate"></el-date-picker>
        <span class="search-delimiter">-</span>
        <el-date-picker v-model="search.lastUpdateEndTime" class="search-component search-input" type="date" placeholder="更新结束时间" value-format="yyyy-MM-dd" :picker-options="endUpdateValidator" :clearable="false" @change="checkStartDate"></el-date-picker>
        <el-select v-model="search.customerType" placeholder="请选择机器人名称" clearable class="search-component">
      ...
      <div class="tool-button">
        ...
        <!-- <el-button
          ...
          :disabled="$store.state.cusListState === 'loading' || !authExport"
          >{{
            $store.state.cusListState === 'loading' ? '正在导出' : '导出客户'
          }}</el-button
        >
        <el-button
    ...
    <div class="table">
      <el-table :data="callList" ref="table" stripe height="100%" border tooltip-effect="dark" v-loading="isLoading" @select="handleSingleSelect" @select-all="handleAllSelect">
        ...
        <el-table-column :resizable="false" prop="name" label="任务名称" width="120" align="center"></el-table-column>
        <el-table-column :resizable="false" prop="robotName" label="机器人名称" width="120" align="center"></el-table-column>
        <el-table-column :resizable="false" prop="concurrentNum" label="并发数量" width="120" align="center"></el-table-column>
        <el-table-column :resizable="false" prop="calls" label="外呼平台" width="120" align="center"></el-table-column>
        ...
        <!-- <el-table-column :resizable="false" prop="status" label="任务状态" width="120" align="center">
          <template slot-scope="scope">
            <span v-show="scope.row.status === 0">待启动</span>
            <span v-show="scope.row.status === 1">运行中</span>
            <span v-show="scope.row.status === 2">已完成</span>
            <span v-show="scope.row.status === 3">已暂停</span>
            <span v-show="scope.row.status === 4">定时暂停</span>
          </template>
        </el-table-column> -->
        <el-table-column :resizable="false" prop="phoneCt" label="号码总数" width="120" align="center"></el-table-column>
        <el-table-column :resizable="false" prop="finishNum" label="已呼数量" width="120" align="center"></el-table-column>
        <el-table-column :resizable="false" prop="successNum" label="接通数量" width="120" align="center"></el-table-column>
        <el-table-column :resizable="false" prop="connectRate" label="接通率" width="120" align="center"></el-table-column>
        <el-table-column :resizable="false" label="定时启动时间" width="110" align="center">
          <template slot-scope="scope">
            <span>{{
        ...
        <el-table-column :resizable="false" label="开始时间" width="110" align="center">
          <template slot-scope="scope">
            <span>{{
        ...
        <el-table-column :resizable="false" label="结束时间" width="110" align="center">
          <template slot-scope="scope">
            <span>{{
        ...
        <el-table-column :resizable="false" label="操作" min-width="240" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button @click="toDetail(scope.row)">启动</el-button>
            <el-button @click="toFollow(scope.row)">下载</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleSingleSelect (selection, row) {
      ...
      const index = this.callList.findIndex((item) => {
    ...
    handleAllSelect (selection) {
      ...
      if (this.isSelectAll && selection.length === 0) {
        this.checkedTableRow = this.callList
          .map((item, index) => {
            return {
              page: this.pagination.currentPage,
              index,
    ...
    toDetail ({ mobile, userId }) {
      ...
      this.$router.push({ path: '/customerManage/customerInfo', query: params })
    },
    //跳转跟进记录
    ...
    queryList () {
      ...
      this.$request
        ...
        .then((res) => {
          ...
          if (res.code == 0) {
            this.callList = res.data ? res.data.list : []
